...
class PersonalNotificationFragment :
    BaseFragment<FragmentPersonalNotificationBinding>(FragmentPersonalNotificationBinding::inflate) {
    ...
    override fun bindData() {
        ...
        binding.rclPersonalNotification.layoutManager =
        ...
        viewModel2.userInformation.observe(viewLifecycleOwner, Observer { informationList ->
            if (informationList == null) {
                ...
                requireActivity().getToken()
                    ?.let { viewModel.getPersonalNotification(memberId, it) }
            }
        })

        viewModel.listPersonalNotification.observe(
            viewLifecycleOwner,
